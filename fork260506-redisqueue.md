# fork260506-redisqueue: Workspace Patch Log
...
> why** each one exists.
>
> Sub-repo upstream remote: see `git -C fork260506-redisqueue remote -v`.
...
- **Applied on disk?** Tick `[x]` when the patch is currently applied to the sub-repo working tree. Untick `[ ]` when reverted (`git -C fork260506-redisqueue checkout <file>`).
